...
#define K 0.04 // 0.22 //0.04   // Harris detector constant
...
// -----------------------------------------------------------------------------
// RDTSC timing routine (used in main) - now comes from nonmaxsup.h
// -----------------------------------------------------------------------------
    ...
    float *restrict op)
{
    ...
    const int UNROLL    = 4;      // 4-way register blocking (loop step is 5*8)
    ...
    for (int r = 0; r < ksize; ++r) {
        ...
        for (int i = 0; i < m_out; ++i) {
            ...
            for (; j + UNROLL * VEC_WIDTH - 1 < n_out; j += UNROLL * VEC_WIDTH) {
                ...
                // Note: j advances by 5*VEC_WIDTH, lane 4 is handled in scalar tail
            }
...
// -----------------------------------------------------------------------------
// Sobel (existing AVX implementation)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Gaussian blur (existing AVX implementation)
// -----------------------------------------------------------------------------
...
void gaussian3(float **src, float **dst, int h, int w) {
    ...
    float g[4] = {
        ...
        2.0f / 4.0f,  // tap 1
        1.0f / 4.0f,  // tap 2
        0.0f          // tap 3 (dummy, keeps ksize=4)
    };
    ...
    int m      = h;
    int n      = w;
    int ksize  = 4;          // <--- IMPORTANT: use 4-tap kernel
    ...
    for (int x = 0; x < w; ++x) {
        ...
        tmp[h - 1][x] = 0.0f;
        if (h >= 2)
            tmp[h - 2][x] = tmp[h - 2][x];  // leave as whatever (or set 0 if you like)
    }
    for (int y = 0; y < h; ++y) {
        tmp[y][0]     = 0.0f;
        tmp[y][w - 1] = 0.0f;
